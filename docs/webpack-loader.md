...
3. 当webpack发现某个文件处理不了的时候，会查找 `webpack.config.js` 这个配置文件，看module.rules数组中，是否配置了对应的loader加载器
4. webpack把 `index.css` 这个文件先转交给最后一个loader进行处理（先转交给css-loader）
...
!!! success "下一步"
    现在你已经掌握了webpack加载器的使用，接下来让我们学习 [打包构建](build.md) 的相关知识。