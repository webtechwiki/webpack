...
通过安装和配置第三方插件，可以拓展 webpack 的能力，从而让 webpack 使用起来更方便。最常用的 webpack 插件有以下两个：
...
实际上，`dev` 指令对应的是 `webpack serve` 命令，执行 `npm run dev` 最终将执行 `webpack serve` 命令，webpack-dev-server 会启动一个实时打包的 http 服务。此时打包后 `bundle.js` 存在于内存中，我们通过 `http://localhost:8080/bundle.js` 可以访问到实时打包的 js 文件。

!!! warning "版本注意事项"
    如果你安装的是新版本的 webpack-dev-server，直接访问 `http://localhost:8080`，可能会看到 `Cannot GET /` 报错，因为新版的 webpack-dev-server 不再默认加载目录。但是通过 `http://localhost:8080/bundle.js` 地址依旧能访问到打包后的 js 文件。
...
!!! important "重启提醒"
    凡是修改了 webpack.config.js 配置文件，或者修改了 package.json 文件，必须重启实时打包服务，否则最新配置文件无法生效。
...
通过以下命令安装指定版本的 html-webpack-plugin
...
此时，我们关闭之前的 http 服务后，再次执行 `npm run dev`，将启动新的 http 服务，这次访问 http 服务，我们直接访问到 web 界面效果，在修改代码之后，web 界面效果也会实时更新。
...
!!! success "下一步"
    现在你已经掌握了webpack插件的使用，接下来让我们学习 [webpack加载器](webpack-loader.md) 的配置和使用。